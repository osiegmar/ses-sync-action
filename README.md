...
```yaml
- uses: osiegmar/ses-sync-action@v2
  with:
    dir: templates
```

The directory `templates` has to contain the following three files for each
template:
...
The template configuration file has to be a JSON file with the following
structure:
